...
${play_introduce}    Media AddChannel normal
${comment_more}    comment discuss more
...
${comment_content}    (//XCUIElementTypeButton[@name="${comment_more}"])[1]//preceding-sibling::XCUIElementTypeOther[last()]
${commentdetail_content}    //XCUIElementTypeStaticText[@name="评论详情"]/../following-sibling::XCUIElementTypeTable/XCUIElementTypeCell[1]/XCUIElementTypeOther

*** Keywords ***
点播播放页-返回
    sleep    2
    comment    公共－点击页面文本    player back Shadow
    click element    nsp= name=="vodplayer back Shadow" and visible==true
    sleep    2

点播播放页-暂停
    tap    xpath=//XCUIElementTypeButton[@name="vodplayer back Shadow"]    100    100    count=2
...
点播播放页-下载
    Wait Until Element Is Visible    ${play_download}    timeout=20
...
新版点播-加入看单
    公共－xpath    //XCUIElementTypeButton[@name="${play_collect}"]
...
点播播放页-花絮
    ...
    公共－xpath    //XCUIElementTypeOther[@name="花絮片段"]/XCUIElementTypeButton
...
新版评论-点更多
    公共－xpath    (//XCUIElementTypeButton[@name="${comment_more}"])[1]

新版评论-点分享
    公共－xpath    (//XCUIElementTypeButton[@name="${comment_more}"])[1]

新版评论-点赞
    公共－xpath    (//XCUIElementTypeButton[@name="${comment_more}"])[1]/preceding-sibling::XCUIElementTypeButton[1]

新版评论-点回复
    公共－xpath    (//XCUIElementTypeButton[@name="${comment_more}"])[1]/preceding-sibling::XCUIElementTypeButton[2]
    验证手机

新版评论-点更多话题
    sleep    2
    公共－xpath    //XCUIElementTypeOther[@name="话题"]/XCUIElementTypeButton

新版评论-点单个话题
    公共－xpath    //XCUIElementTypeButton[@name="#青春芒果节#"]

新版评论-点评论者头像
    公共－xpath    (//XCUIElementTypeButton[@name="${comment_more}"])[1]/preceding-sibling::XCUIElementTypeImage[1]

新版评论-点评论中话题
    点评论中话题    ${comment_content}

评论详情页-点评论中的话题
    点评论中话题    ${commentdetail_content}

点评论中话题
    [Arguments]    ${xpath}
    &{size} =    get element size    xpath=${xpath}
    ${x} =    evaluate    ${size.width} -100
    tap    xpath=${xpath}    ${x}    15

新版评论-点评论内容
    公共－xpath    ${comment_content}
...
新版评论详情页-点回复
    公共－xpath    //XCUIElementTypeButton[@name="${comment_more}" and @visible="true"]/preceding-sibling::XCUIElementTypeButton[2]
    验证手机

新版评论详情页-点赞
    公共－xpath    //XCUIElementTypeButton[@name="${comment_more}" and @visible="true"]/preceding-sibling::XCUIElementTypeButton[1]

新版评论详情页-点分享
    公共－xpath    //XCUIElementTypeButton[@name="${comment_more}" and @visible="true"]
